...
def format_iteration(console: Console, number: int, goal: str, decision: str) -> None:
    ...
    table.add_row("Iteration", f"[{theme.HEADER}]{number}[/]")
    table.add_row("Goal", goal)
    table.add_row("Decision", f"[{theme.INFO}]{decision}[/]")
...
def format_check_results(console: Console, results: dict[str, Any]) -> None:
    ...
    table.add_column("Check", style=theme.INFO)
    table.add_column("Status")
    table.add_column("Duration")

    for check_name, result in results.items():
        status_style = theme.SUCCESS if result["status"] == "pass" else theme.ERROR
        table.add_row(
            check_name,

...
async def derive_code_async(
    ...
    if not payload_file.exists():
        console.print(f"[{theme.ERROR}]Handoff payload not found: {payload_file}[/]")
        console.print(f"[{theme.DIM}]Expected derive_payload.json from research pipeline.[/]")
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(f"[{theme.ERROR}]Invalid JSON in handoff payload: {e}[/]")
        raise typer.Exit(1) from e

    console.print(f"[{theme.INFO_BOLD}]Loading research handoff:[/] {payload_file}")
    ...
    # Show summary
    console.print(f"\n[{theme.HEADER}]Headline:[/] {headline}")
    console.print(f"[{theme.HEADER}]Goals:[/] {', '.join(goals) if goals else 'None specified'}")
    if recommended_approach:
        console.print(f"[{theme.HEADER}]Approach:[/] {recommended_approach}")
    console.print(f"[{theme.HEADER}]Key findings:[/] {len(key_findings)}")
    if risks:
        console.print(f"[{theme.HEADER}]Risks:[/] {len(risks)}")
    ...
    if workspace is None:
        if workspace_hint:
            ...
            console.print(f"[{theme.ERROR}]Error: --workspace is required (no hint in payload)[/]")
            raise typer.Exit(1)

    if not workspace.exists():
        console.print(f"[{theme.ERROR}]Workspace not found: {workspace}[/]")
        raise typer.Exit(1)
    ...
    console.print(f"\n[{theme.DIM}]Workspace: {workspace.absolute()}[/]")
    ...
    # Stages after MCP (step 2) need offset since derive_code doesn't use MCP
    ...
    def stage_callback(name: str, is_starting: bool, duration: float) -> None:
        ...
        if is_starting:
            step_offset = 1 if name in stages_after_mcp else 0
            format_stage_header(console, name, total_steps=6, step_offset=step_offset)
        else:
    ...
    try:
        ...
        logger.error(f"Derive-code task failed: {e}")

...
class JsonTaskRepo(TaskRepoPort):
    ...
    async def save(self, task: Task) -> None:
        ...
        """
        task_dir = self._paths.task_dir(task.id)
        task_dir.mkdir(parents=True, exist_ok=True)

        task_path = task_dir / "task.json"
        await atomic_write(task_path, task.model_dump_json(indent=2))
    ...
    async def save_conditions_approval(
        ...
        await atomic_write(conditions_path, json.dumps(existing, indent=2))
    ...
    async def save_plan_approval(self, task_id: UUID, plan: Plan) -> None:
        ...
        await atomic_write(plan_path, json.dumps(existing, indent=2))
    ...
    async def save_inventory(
        ...
        await atomic_write(inventory_path, inventory.model_dump_json(indent=2))

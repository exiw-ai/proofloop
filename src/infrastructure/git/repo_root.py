...
async def get_default_state_dir(path: str | Path = ".") -> Path:
    """Return global state directory (XDG compliant).

    Uses $XDG_DATA_HOME/proofloop or ~/.local/share/proofloop. The path
    parameter is ignored but kept for backwards compatibility.
    """
    _ = path  # unused, kept for API compatibility
    return get_xdg_data_home() / "proofloop"

...
class TestArtifactStorePaths:
    ...
    def test_iteration_dir_formatting(self, artifact_store: ArtifactStore, tmp_path: Path) -> None:
        ...
        assert artifact_store._iteration_dir(task_id, 99) == expected
...
class TestAppendTimeline:
    ...
    async def test_append_timeline_creates_file(self, artifact_store: ArtifactStore) -> None:
        ...
        timeline_path = artifact_store._task_dir(task_id) / "timeline.jsonl"
...
class TestSaveIteration:
    ...
    async def test_save_iteration(self, artifact_store: ArtifactStore) -> None:
        ...
        iteration_path = artifact_store._iteration_dir(task_id, 1) / "iteration.json"
    ...
    async def test_save_multiple_iterations(self, artifact_store: ArtifactStore) -> None:
        ...
        iter1_path = artifact_store._iteration_dir(task_id, 1) / "iteration.json"
        iter2_path = artifact_store._iteration_dir(task_id, 2) / "iteration.json"
...
class TestSaveAgentEvents:
    ...
    async def test_save_agent_events(self, artifact_store: ArtifactStore) -> None:
        ...
        events_path = artifact_store._iteration_dir(task_id, 1) / "agent" / "events.jsonl"
...
class TestSaveAgentTranscript:
    ...
    async def test_save_agent_transcript(self, artifact_store: ArtifactStore) -> None:
        ...
        transcript_path = artifact_store._iteration_dir(task_id, 1) / "agent" / "transcript.md"
...
class TestSaveDiff:
    ...
    async def test_save_diff(self, artifact_store: ArtifactStore) -> None:
        ...
        diffs_dir = artifact_store._iteration_dir(task_id, 1) / "diffs"
...
class TestSaveFinalResult:
    ...
    async def test_save_final_result(self, artifact_store: ArtifactStore) -> None:
        ...
        final_dir = artifact_store._task_dir(task_id) / "final"
...
class TestSaveCache:
    ...
    async def test_save_cache(self, artifact_store: ArtifactStore) -> None:
        ...
        cache_path = artifact_store._task_dir(task_id) / "cache" / f"{key}.json"
    ...
    async def test_save_multiple_caches(self, artifact_store: ArtifactStore) -> None:
        ...
        cache_dir = artifact_store._task_dir(task_id) / "cache"
...
class TestAtomicWrite:
    ...
    @pytest.mark.asyncio
    async def test_atomic_write_creates_parent_dirs(
        self, artifact_store: ArtifactStore, tmp_path: Path
    ) -> None:
        """Test atomic write creates parent directories."""
        path = tmp_path / "nested" / "deep" / "file.txt"

        await artifact_store._atomic_write(path, "content")

        assert path.exists()
        assert path.read_text() == "content"

    @pytest.mark.asyncio
    async def test_atomic_write_overwrites(
        self, artifact_store: ArtifactStore, tmp_path: Path
    ) -> None:
        ...
        await artifact_store._atomic_write(path, "new content")

...
class TestToolFormatter:
    ...
    def test_format_tool_use_shows_description_as_thought(self) -> None:
        console = make_console()
        msg = AgentMessage(
            role="tool_use",
            content="",
        ...
        format_tool_use(console, msg)
        output = get_output(console)
        # Description should appear as thought before tool operation
    ...
    def test_format_tool_result_truncates_long_content(self) -> None:
        ...
        # Bash results show line count
        format_tool_result(console, msg, tool_name="Bash")
        output = get_output(console)
        assert "100" in output
        assert "lines output" in output
    ...
    def test_create_tool_callback_shows_assistant_as_thought(self) -> None:
        console = make_console()
        callback = create_tool_callback(console)

        # Assistant messages are shown as thoughts (agent describing what it's doing)
        msg = AgentMessage(role="assistant", content="Analyzing the code structure...")
        callback(msg)

        output = get_output(console)
        # Assistant messages displayed as thoughts
        assert "Analyzing the code structure" in output

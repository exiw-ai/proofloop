...
class TestVerifyResearchConditionsRun:
    ...
    async def test_transitions_to_conditions_status(
        ...
        use_case = VerifyResearchConditions(kb_store, report_store, tmp_path)
    ...
    async def test_returns_error_without_inventory(
        self, kb_store: KnowledgeBaseStore, report_store: ReportPackStore, tmp_path: Path
    ) -> None:
        task = Task(
            id=uuid4(),
            description="Research patterns",
    ...
    async def test_checks_synthesis_passes_pass(
        ...
        (synthesis_dir / "synthesis_log.json").write_text(
            json.dumps({"passed": True, "completed_passes": 1})
        )
...
class TestCountByType:
    def test_counts_single_type(self) -> None:
        ...
        use_case = VerifyResearchConditions(MagicMock(), MagicMock(), Path("."))

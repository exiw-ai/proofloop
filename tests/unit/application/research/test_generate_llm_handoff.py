...
class TestGenerateLLMHandoffRun:
    ...
    async def test_generates_handoff(
        ...
        use_case = GenerateLLMHandoff(
            mock_agent, mock_kb_store, mock_report_store, mock_handoff_store
        )
    ...
    async def test_includes_repo_context_if_exists(
        ...
        # Create repo_context manifest
        repo_context_dir = tmp_path / ".proofloop" / "research" / "repo_context"
        repo_context_dir.mkdir(parents=True)
        (repo_context_dir / "manifest.json").write_text("{}")

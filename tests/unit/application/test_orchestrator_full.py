...
class TestDiscoverWorkspace:
    ...
    async def test_discover_workspace_sets_workspace_info(self, orchestrator, tmp_path):
        """discover_workspace should set _workspace_info on
        WorkspaceManager."""
        with patch.object(
            ...
            result = await orchestrator._workspace_manager.discover_workspace(tmp_path)

            assert result.is_workspace is False
            assert result.root == tmp_path
            assert orchestrator._workspace_manager.workspace_info == result

    @pytest.mark.asyncio
    async def test_discover_workspace_multiple_repos(self, orchestrator, tmp_path):
        """discover_workspace should handle multiple repos."""
...
class TestStashAllRepos:
    ...
    async def test_stash_all_repos_calls_diff_port(self, orchestrator, mock_diff_port, tmp_path):
        """stash_all_repos should call diff_port.stash_all_repos."""
        ...
        await orchestrator._workspace_manager.stash_all_repos("Test stash")
    ...
    async def test_stash_all_repos_no_workspace_info(self, orchestrator, mock_diff_port):
        """stash_all_repos should do nothing if no workspace_info."""
        orchestrator._workspace_manager._workspace_info = None
    ...
    async def test_stash_all_repos_handles_failure(self, orchestrator, mock_diff_port, tmp_path):
        """stash_all_repos should handle stash failures gracefully."""
        orchestrator._workspace_manager._workspace_info = WorkspaceInfo(
            is_workspace=False,
            repos=[tmp_path],
            root=tmp_path,
...
class TestRollbackAllRepos:
    ...
    async def test_rollback_all_repos_calls_diff_port(self, orchestrator, mock_diff_port, tmp_path):
        """rollback_all_repos should call diff_port.rollback_all."""
        orchestrator._workspace_manager._workspace_info = WorkspaceInfo(
            is_workspace=False,
            repos=[tmp_path],
            root=tmp_path,
        ...
        await orchestrator._workspace_manager.rollback_all_repos("Rollback message")
    ...
    async def test_rollback_all_repos_no_workspace_info(self, orchestrator, mock_diff_port):
        """rollback_all_repos should do nothing if no workspace_info."""
        orchestrator._workspace_manager._workspace_info = None
                ...
                with (
                    ...
                    with patch.object(
                        ...
                        await orchestrator.run(
                            task_input,
                            callbacks=OrchestrationCallbacks(on_stage=on_stage),
                        )
                ...
                with (
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            await orchestrator.run(
                                task_input,
                                callbacks=OrchestrationCallbacks(plan_and_conditions=plan_callback),
                            )
                            ...
                            with patch.object(
                                ...
                                await orchestrator.run(
                                    ...
                                    callbacks=OrchestrationCallbacks(
                                        plan_and_conditions=plan_callback
                                    ),
                                )
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            result = await orchestrator.run(
                                task_input,
                                callbacks=OrchestrationCallbacks(plan_approval=deprecated_callback),
                            )
            ...
            with patch.object(
                ...
                with (
                    ...
                    with patch.object(
                        ...
                        result = await orchestrator.run(
                            task_input,
                            callbacks=OrchestrationCallbacks(plan_and_conditions=reject_callback),
                        )
